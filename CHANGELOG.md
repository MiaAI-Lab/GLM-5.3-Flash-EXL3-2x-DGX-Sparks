...
## Unreleased — opt-in mixed-prefill gate (warm bypass + deadline, installer v6)

`overlay/patch_scheduler_decode_floor.py` (installer `# [glm53-decode-floor:v6]`)
adds two operator knobs to the `skip`/`cap` paths, both **off by default**, so
the fair v5 default, `skip`, `cap` and `off` are unchanged until an operator
opts in:

- `GLM53_MIXED_PREFILL_WARM_TOKENS` (0 = off) admits a request whose uncached
