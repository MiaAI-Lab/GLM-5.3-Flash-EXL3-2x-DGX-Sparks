...
1. api_server.py — ASGI middleware that normalizes Codex-private item types
   ...
   buffers the request body, normalizes it, replays the (possibly
   rewritten) body once, then DELEGATES to the real receive so uvicorn
   still observes http.disconnect. (The abandoned v1 design returned
   http.disconnect unconditionally, which aborted mid-stream responses
   with a null body.)
...
if "_CodexCompatMiddleware" in src and "__codex_compat_v2" not in src:
    raise SystemExit(
        "codex-compat: api_server carries a PRE-V2 _CodexCompatMiddleware "
        "(baked before the receive-delegate fix). Refusing to silently skip: "
        "the old variant aborts /v1/responses with a null body. Rebuild the "
        "image or strip the baked patch, then re-run.")
...
__codex_compat_v2 = "receive-delegate"  # idempotency/version marker
    ...
    async def __call__(self, scope, receive, send):
        if (scope.get("type") == "http" and scope.get("method") == "POST"
            ...
            async def _receive():
                ...
                # Delegate to the real upstream receive(): returning
                # http.disconnect here makes the app abort mid-stream and the
                # endpoint answers with a null body.
