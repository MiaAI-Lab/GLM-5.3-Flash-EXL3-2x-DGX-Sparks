...
**Scope (added 2026-09-15).** This PR adds only this document and the three
probes in §1. Every launcher, overlay, `.env`, README and warmup change named
below was made in the author's local investigation checkout or belongs to a
separate, unmerged PR, and is labeled as such where it appears; none of it is in
this PR. The stack comparisons are recorded as whole-stack observations, not as
single-mechanism attributions, and nothing here replaces a guard or overlay that
main already ships.
...
| 3 | Long-prefill boot warmup rung | **fixed in separate, unmerged PR #170 — not this PR** | the only mid-serve JIT in the prior 5 h serve was `BuildPrefillChunkMetadataKernel`; rungs added, ARG_MAX staging fixed |
...
attention-token pool. **Update 2026-09-11 (later): the cliff stopped
reproducing on this kit after a whole-stack change (per-group retention +
fine-grained 64-token hits + DFlash block-drop), re-measured live — see §12.
That contrast is recorded as a whole-stack observation and does not isolate the
responsible mechanism (§12.3).** Boot B3 (per-group
...
harmless. `scripts/boot-shape-warmup.sh` in the local investigation checkout
pre-warms prefill metadata with `PREFILL_S=(3584 7168 14336 65536)` (64 k covers
agent-sized contexts) and stages large payloads through a file — the first 64 k
rung hit `curl: Argument list too long`, fixed there. Those rungs are carried by
**separate, unmerged PR #170** (not this PR; current main's script has no
`PREFILL_S` rungs). B2 verifies coverage in that checkout.
...
## 11. Changes made in the local investigation checkout (none of them in this PR)
...
- `scripts/boot-shape-warmup.sh` — long-prefill rungs
  ...
  outcome tally now ignores the staged `.json` payload files. Carried by
  **separate, unmerged PR #170**; absent from this PR's head and from main.
- `start.sh` — adaptive-k auto-selects the union capture list; forwards
  ...
  overlays. Local checkout state: the artifact gate, `GLM53_OVERLAY_ORDER` and
  `emit_overlay_block` are on main via PR #130; the `GLM53_FINEGRAINED_APC`
  plumbing is not (it belongs to unmerged PR #84).
- `.env` / `.env.example` — adaptive-k `ema`, spinwait 16, dense FP8 (this kit),
  ...
  retention knob documentation. `.env` is untracked kit config; of the
  `.env.example` entries only `GLM53_APC_RETENTION_INTERVAL_SWA` is on main
  (via #130) — `GLM53_FINEGRAINED_APC` belongs to unmerged PR #84.
- `README.md`, `CHANGELOG.md` — new defaults, measurements, and the fixed
  prefix-cache section. Local checkout only; main's README and CHANGELOG do not
  carry these edits.
- Adopted prefix-cache fixes (local checkout / unmerged PRs, none in this PR):
  `overlay/patch_apc_per_group_retention.py` — the PR #83 implementation; main
  ships the #130 rework of this approach instead (§12.3), and its
  `docs/DESIGN-apc-per-group-retention.md` + `tests/test_launcher_rank_parity.py`
  are already on main. `overlay/patch_apc_fine_grained_hits.py` — unmerged PR #84
  (+ design doc + test). `overlay/patch_dflash_block_drop.py` — local cherry-pick
  of unmerged vLLM PR #54163 (+ test). The last two files are not in this PR's
  head.
...
The last two entries are what this PR contains. The local `.env` carried the
full recommended configuration; boot B3 validated it live in that checkout.
...
The upstream fingerprints are consistent with what was observed here: the cliff
was not a client-side prompt fork (§4.2), not explained by the attention-token
pool or the global retention interval (§4.4), and moved only with the whole
retention/replay stack — recorded as a cross-check across stacks, not as an
isolated attribution.

### 12.1 What was adopted in the local checkout

Adopted on 2026-09-11 in the author's checkout, not in this PR; merge status as
of 2026-09-15 is marked per item.
...
- **PR #83** (`overlay/patch_apc_per_group_retention.py`, knob
  ...
  That file is also on main, as the #130 rework (§12.3).
- **PR #84** (`overlay/patch_apc_fine_grained_hits.py`, `GLM53_FINEGRAINED_APC=1`,
  ...
  prefix-cache hits ENABLED … effective alignment=64 tokens`. **Unmerged**
  (PR #84); not in this PR's head.
- **vLLM #54163 cherry-pick** (`overlay/patch_dflash_block_drop.py`): DFlash/DSpark
  ...
  **Upstream unmerged**; local cherry-pick only, not in this PR's head.
- Launcher hardening from #83/#84: `validate_overlay_artifacts` fail-closed gate,
  ...
  0/1 validation, `GLM53_APC_RETENTION_INTERVAL_SWA` grid validation. The first
  two are on main via #130; the fine-grained knob validation is local (#84).

Host tests in that checkout (all pass): `test_launcher_rank_parity.py`,
...
### 12.2 Live A/B (same probes, whole-stack before/after)
...
Before/after here is a whole-stack change (per-group retention + fine-grained
hits + block-drop + the local launcher), recorded as an observation, not a
single-variable comparison.

Boot B3 ran that checkout's `.env`: adaptive-k `ema`, spinwait 16, dense FP8,
...
### 12.3 Merge reconciliation A/B (origin PR #130 vs the local checkout stack)

`origin/main` advanced to `9348755` (PR #130, a rework of #83's per-group
retention plus a versioned DFlash SWA replay clamp — the replay guard main
ships). Merging it into the local checkout surfaced a functional conflict, so it
was resolved there by measurement, not by text. Four consecutive boots on this
kit, same five probes:
...
The B3–B7 table is recorded exactly as observed. B3 and B4–B6 are *different
overlay stacks*, not a single-variable retention comparison: B5/B6 set the SWA
interval explicitly to 0/0 (not an unset-versus-enabled-default comparison), and
B6 with the block-drop patch removed reproduces B5's pattern. So the contrast
does not by itself identify the responsible mechanism, and it is not a basis for
replacing the replay guard main ships (#130's versioned DFlash SWA replay
clamp); the isolated runtime comparison and the merge decision remain with the
maintainers. (Upstream's qualification doc explicitly frames #130 as "a draft
contribution for maintainer discussion, not a recommendation", and its measured
cases are 128k append/edit/branch, not identical-repeat growth.) **Local
checkout resolution (2026-09-11, not this PR):** that checkout kept the #83
overlay implementations (live-validated above) and took PR #130's launcher
machinery, artifact gate, overlay order, tests and docs; the overlay files listed
in §12.1 and their host tests are its functional divergence, recorded there and
in its `.env.example`. This PR carries none of that. Boot **B7** then
re-validated the restored stack on the merged launcher: 1×/3×/4× 30 k repeats
**1.000** at 0.3–0.4 s, 2×60 k **0.999**, 3×5 k **0.992**, and the 4-agent ×
60 k bench turns 1–3 at **0.886–0.928** (wall 17.7–23.6 s), matching B3.
