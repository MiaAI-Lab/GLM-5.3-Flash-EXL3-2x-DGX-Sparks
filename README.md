...
| `GLM53_MIXED_PREFILL_WARM_TOKENS` | `0` (off) | Gate v6 opt-in, `skip`/`cap` only: a request whose **uncached remainder** is at most this many tokens is admitted at once instead of waiting behind a decoder. `0` disables the bypass — it is not an "everything is warm" mode, so the smallest remainder is still held. `3584` (one hybrid block) admits cached follow-up tails, short cold prompts and the last block of a late-capped prefill. `fair` is unaffected. |
| `GLM53_MIXED_PREFILL_MAX_WAIT_MS` | `0` (wait forever) | Gate v6 opt-in, `skip` only: after the skip hold has starved a request for this long (`1..600000` ms; monotonic first-seen stamp that survives chunking, preemption and requeue) the request becomes eligible for a `GLM53_MIXED_PREFILL_LATE_CAP` step. **Eligibility only, not a bound**: it reserves no allocation and bounds neither admission nor service time nor TTFT — capacity and the base scheduler's own limits still decide when the request is served, and once served it crawls like `cap:N` (a policy statement about the released chunk, not a measured latency or throughput figure; no live qualification of this knob was performed here). `0` keeps the v5 behaviour of waiting until the decoders finish. |
| `GLM53_MIXED_PREFILL_LATE_CAP` | `512` | Per-step prefill tokens once a held request passes `GLM53_MIXED_PREFILL_MAX_WAIT_MS` (`64..8192`; applied only while the deadline is enabled, and never more than the request's remaining prefill). An explicit `cap:N` keeps its own cap. |
