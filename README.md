...
| `DFLASH_SCHEDULE` | *(unset)* | DFlash2 dynamic draft taper, JSON `[[batch_lo,batch_hi,k],…]` → `num_speculative_tokens_per_batch_size` (already honoured per step by this image's scheduler). Measured 2026-09-19 (k=3 + dense FP8, pure decode): c8 `k=1` **91.9 tok/s** agg at 98.6 % of the 236 GB/s DRAM ceiling vs `k=0` 75.3 — speculation is +40 % bytes for +70 % tokens on a bandwidth-bound MoE. `tests/bench_ceiling.py` measures this; `tests/bench_c8.py` is the agent-shaped c1…c8 ladder; `tests/bench_nvme_restore.py` the restart-restore receipt |
| `OFFLOAD_NVME` | `0` | `1` = NVMe-direct prefix cache (`overlay/kvoffload/`): vLLM `OffloadingConnector` + out-of-tree `NvmeDirectOffloadingSpec`, no CPU tier, per-rank file trees under `OFFLOAD_HOST_DIR` / `WORKER_OFFLOAD_HOST_DIR` mounted at `OFFLOAD_FS_DIR`, restart-restore (46,796-token prompt: 32.4 s cold → **1.58 s** TTFT after an engine restart, `cached_tokens` 46,592). `OFFLOAD_IO_THREADS` (8), `OFFLOAD_CAPACITY_BYTES` (empty = min free of both nodes − `OFFLOAD_RESERVE_GB` 24), `OFFLOAD_TTL_MINUTES` (0 = no sweeper). Adds `--enable-prompt-tokens-details`, `PYTHONHASHSEED=0`, strips `expandable_segments`. See [docs/nvme-prefix-cache.md](docs/nvme-prefix-cache.md) |
| `EXTRA_MOUNT` | *(unset)* | one extra docker `-v` bind on both ranks (same host path on both nodes), e.g. a read-only tree the HF snapshot symlinks into |
| `GLM53_HOST_MEM_HYGIENE` | `1` | drop clean page cache + cycle swap on both nodes right before `docker run`, then wait for CUDA free memory to clear `GPU_MEM_UTIL` × total (`sudo -n`; warns without it). On GB10 UMA the page cache is "used" CUDA memory: a full cache after the 164 GiB rsync made InstantTensor abort (`buffer_size … exceeds device memory budget`) or crawl at a double-digit `io_depth`. Pairs with `overlay/patch_cold_load_uma.py` (in-container budget + 64 KiB mmap staging; kill switch `GLM53_COLD_LOAD_UMA=0`; no-op on 4 KiB kernels / discrete GPUs). See [docs/cold-load-uma.md](docs/cold-load-uma.md) |
...
| `overlay/kvoffload/` | NVMe-direct prefix cache: `nvme_direct.py` spec, draft-group detection + `KpoolTailSpec` exclusion in the offloading scheduler/config |
| `Dockerfile.nvme-layer` | cold-load + NVMe cache layered on the published `:exl3-instanttensor` image without the exllamav3 recompile |
