...
| `GLM53_APC_RETENTION_INTERVAL_SWA` | (auto) | per-group prefix-cache retention for the DFlash2 drafter's sliding-window group (overlay `patch_apc_per_group_retention.py`). Root cause of the two-long-conversation eviction: a cached 3584-token segment costs 38 block ids of which 33 are drafter blocks that are hashed and freed mid-prefill to the LRU tail, ahead of the MLA/mamba blocks that carry the hit. Auto/`0` = keep drafter snapshots only at reachable boundaries → ≈5 ids per segment (≈54 segments ≈ 190K tokens per conversation) while MLA/mamba stay on the dense 3584 grid; `N` = multiple of 3584. A non-empty value requires `SPEC_METHOD=dflash`; the launcher rejects incompatible modes before stopping a healthy deployment. Boot log: `[glm53-apc-per-group] retention_by_group=[None,…,None,0]`. Refuses to boot if set without `patch_hybrid_prefix_hit.py` (EAGLE min-exemption required) |
