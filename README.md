...
| `GLM53_MIXED_PREFILL_MAX_WAIT_MS` | `0` (wait forever) | Gate v6 opt-in, `skip` only: release a request the skip hold has starved for this long (`1..600000` ms; monotonic first-seen stamp that survives chunking, preemption and requeue) under `GLM53_MIXED_PREFILL_LATE_CAP` tokens per step. **This bounds time-to-first-service (admission), not TTFT or completion**: the released request then crawls like `cap:N` and slows running decodes for its duration — the reviewed gate work reported roughly a 5× decode slowdown in that window, which the CPU checks here do **not** reproduce and no live run in this round re-measured. `0` keeps the v5 behaviour of waiting until the decoders finish. |
