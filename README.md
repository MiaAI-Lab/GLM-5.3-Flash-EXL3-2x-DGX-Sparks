...
| `overlay/patch_kv_offload_store_local.py` | stage-1 store-only disk KV tier (`GLM53_KV_OFFLOAD=1`): worker-local per-rank chunk files with versioned headers + segment tables, boundary manifests, inline keep-K retention; stage-2 unlocks `GLM53_KV_OFFLOAD_RESTORE=1` (requires `GLM53_KV_OFFLOAD=1`; see the restore overlay row); see `docs/DESIGN-kv-offload-store-only.md` |
