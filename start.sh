...
# Gate v6 (`skip`/`cap` only; both features OFF unless set -- 0 disables each
# one, so fair and the current skip policy are unchanged). WARM_TOKENS admits a
# request whose uncached remainder fits in that many tokens; MAX_WAIT_MS
# releases a request the skip hold has starved, under LATE_CAP tokens/step.
# The release bounds time-to-first-service, not TTFT: it crawls like cap:N.
# Default applies only when UNSET: an explicitly empty value is an operator
# error and validate_numeric_config rejects it.
...
# Same canonicalization, but 0 is valid: the mixed-prefill gate v6 knobs use 0
# to mean "feature off", so a positive-only check would reject their default.
_glm53_canonical_nonnegative_int() {
    local name="$1" value="$2" maximum="$3" canonical
    if ! [[ "$value" =~ ^[0-9]+$ ]]; then
        echo "$name must be a base-10 integer between 0 and $maximum (got: $value)" >&2
        return 2
    fi
    canonical="$value"
    while [ "${canonical#0}" != "$canonical" ]; do canonical="${canonical#0}"; done
    [ -n "$canonical" ] || canonical=0
    if [ "${#canonical}" -gt "${#maximum}" ] \
       || [ "$canonical" -gt "$maximum" ]; then
        echo "$name must be a base-10 integer between 0 and $maximum (got: $value)" >&2
        return 2
    fi
...
# Companion fair and gate-v6 knobs are validated when set so a typo cannot
# reach one rank. The two v6 features are off at 0, which is their default.
_glm53_validate_mixed_prefill() {
    ...
    # Gate v6: 0 disables the warm bypass and the deadline, so these two use the
    # non-negative guard (the positive-int one would reject their default).
    _glm53_canonical_nonnegative_int \
        GLM53_MIXED_PREFILL_WARM_TOKENS "$GLM53_MIXED_PREFILL_WARM_TOKENS" 1000000 || return
    _glm53_canonical_nonnegative_int \
        GLM53_MIXED_PREFILL_MAX_WAIT_MS "$GLM53_MIXED_PREFILL_MAX_WAIT_MS" 600000 || return
    # LATE_CAP only applies while the deadline is enabled; it is bounded like
    # the other per-step caps so a typo cannot turn the release into a full chunk.
    _glm53_canonical_positive_int \
        GLM53_MIXED_PREFILL_LATE_CAP "$GLM53_MIXED_PREFILL_LATE_CAP" 8192 || return
    if [ "$GLM53_MIXED_PREFILL_LATE_CAP" -lt 64 ]; then
        echo "GLM53_MIXED_PREFILL_LATE_CAP must be between 64 and 8192 (got: $GLM53_MIXED_PREFILL_LATE_CAP)" >&2
        return 2
    fi
}
...
start() {
    ...
    log "mixed-prefill: policy=${GLM53_MIXED_PREFILL_CHUNK} fair_chunk=${GLM53_FAIR_PREFILL_CHUNK} share=${GLM53_FAIR_PREFILL_SHARE} interval_ms=${GLM53_FAIR_PREFILL_MAX_INTERVAL_MS} max_step_ms=${GLM53_FAIR_PREFILL_MAX_STEP_MS} max_chunks=${GLM53_FAIR_PREFILL_MAX_CHUNKS} warm_tokens=${GLM53_MIXED_PREFILL_WARM_TOKENS} max_wait_ms=${GLM53_MIXED_PREFILL_MAX_WAIT_MS} late_cap=${GLM53_MIXED_PREFILL_LATE_CAP} long_prefill=${LONG_PREFILL_TOKEN_THRESHOLD:-}"
